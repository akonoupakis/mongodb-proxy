...
Store.prototype.post = function (fn, cb) {
    ...
    getCollection(this.db, this.name, function (err, col) {
        ...
        self.db.run(self, 'precreate', null, processed.data, function (preErr, preResults) {
            ...
            self.run('post', null, preResults, function (postErr, postResults) {
                if (postErr)
                    return error(postErr);
        ...
        if (_.isObject(obj)) {
            for (var name in obj) {
                ...
                if (name === 'id' && obj.id) {
                    ...
                    delete obj.id;
                    if (!_.isArray(value) && _.isObject(obj._id) && !uuid.isValid(obj._id))
                        scrubObj(obj._id);
                }
                else if (!_.isArray(value) && uuid.isValid(value)) {
                    ...
                    if (_.all(value, function (x) {
                        ...
                        return _.isObject(x) && !uuid.isValid(x);
                    })) {
        ...
        if (_.isObject(obj)) {
            for (var name in obj) {
                ...
                if (name === '_id' && obj._id) {
                    ...
                    delete obj._id;
                    if (!_.isArray(value) && _.isObject(obj.id))
                        scrubObj(obj.id);
                }
                else if (!_.isArray(value) && uuid.isValid(value)) {
                    ...
                    if (_.all(value, function (x) {
                        return x && !_.isArray(x) && uuid.isValid(x);
                    })) {
