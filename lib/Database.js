...
var Database = function (options) {
    ...
    this.events = {
        preread: [/*function (sender, collection, context) {
            console.log('preread');
            context.done();
        }, function (sender, collection, context) {
            console.log('preread2');
            context.done();
        }*/],
        postread: [/*function (sender, collection, context, data) {
            console.log('postread1', data.test, data);
            data.test = "test1";
            context.done();
        }, function (sender, collection, context, data) {
            console.log('postread2', data.test, data);
            data.test = "test2";
            context.done();
        }, function (sender, collection, context, data) {
            console.log('postread3', data.test, data);
            data.test = "test3";
            context.done();
        }*/],
        precreate: [function (sender, collection, context, data) {
            console.log('users precreate', data);
            context.done();
        }],
        postcreate: [purgeCacheFn, function (sender, collection, context, data) {
            console.log('users postcreate', data);
            context.done();
        }],
        preupdate: [function (sender, collection, context, data) {
            
            context.done();
        }],
        postupdate: [purgeCacheFn, function (sender, collection, context, data) {
           // console.log('users postput', data);
            context.done();
        }],
        predelete: [],
...
Database.prototype.createStore = function (name, internal) {
    var store = new Store(this, name, internal);
    return store;
};

Database.prototype.createApi = function (internal) {
    var api = new Api(this, internal);
    return api;
};
        ...
        var taskProcess = function (d, prev, tcb) {
            ...
            _.each(self.events[event], function (eventFn) {
                eventTasks.push(function (tcbb) {
                    var context = new Context(store, d, prev, tcbb);
